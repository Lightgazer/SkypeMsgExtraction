...
namespace sqliteSkype
{
	...
	class ChatManager
	{
		...
		public ChatManager (string chat)
		{
			chatname = chat;
	...
	class MainClass
	{
		...
		public static void SubMain (string maindb)
		{
			...
			connection.Open ();
			var getchatlist = (IDbCommand)new SqliteCommand ("SELECT DISTINCT chatname FROM Messages", (SqliteConnection)connection);
			var chatlist_reader = getchatlist.ExecuteReader ();

			while (chatlist_reader.Read ()) {
				var getmsg = (IDbCommand)new SqliteCommand ("SELECT author, timestamp, from_dispname, body_xml, type " +
				             "FROM Messages WHERE chatname='" + chatlist_reader ["chatname"] + "' ORDER BY timestamp", (SqliteConnection)connection);
				var msg_reader = getmsg.ExecuteReader ();
				var chat = new ChatManager (chatlist_reader ["chatname"].ToString ());
				while (msg_reader.Read ()) {
					var dt = TimeStampToDateTime (msg_reader ["timestamp"]);
					chat.AddMsg (dt, msg_reader ["author"].ToString (), msg_reader ["from_dispname"].ToString (), msg_reader ["body_xml"].ToString ());
				}
				chat.Close ();
				chat.PrintSats ();
			}
		...
		public static void Main (string[] args)
		{
			if (File.Exists ("main.db"))
				...
				//search for main.db for all skype accounts
